...
function makeCircle(id){
    ...
    circle.id = "#" + id;
    circle.x = Math.random() * maxX + circleRadius;   //Typo #2 maxX was maXX//
    circle.y = Math.random() * maxY + circleRadius;
    circle.speedX = decideSpeed();
    circle.speedY = decideSpeed();                 //Typo #3 decideSpeed was desideSpeed//
...
function getId(number){
    return "circle" + number;  //Missing Symbol or in this case unneeded symbols #3, there was a () after line 60//
}

// this adds a circle into the HTML
function addNewCircleElement(circle, id){    //Typo #1, function was funtion//
...
function update(){      //Missing Symbol #1, update() was update)//
    for (var i = 0; i < maxCircles; i++){
        var circle = circles[i];       //typo #4 circles[i] was circles[j]//
        moveCircle(circle);
        bounceCircle(circle);
...
function bounceCircle(circle){       
    if (circle.x < 0){                    //Missing Symbol #2 (circle.x <0) was (circle.x <0//
        circle.x -= circle.speedX;
        circle.speedX *= -1;
    }
